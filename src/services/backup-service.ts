...
// The rules that decide whether an archive is restorable are pure and live in
// the domain layer; this module does the parts that touch the filesystem.
import {
  ARCHIVE_DB_FILENAME,
  ARCHIVE_IMAGES_DIRNAME,
  BACKUP_VERSION,
  LEGACY_PAYLOAD_NAME,
  MANIFEST_NAME,
...
// The live database and its copy inside an archive share a name. Both come
// from the domain module rather than being repeated here, because the refusal
// messages name them and a second copy could drift out of agreement with what
// the archive is actually searched for.
const DB_FILENAME = ARCHIVE_DB_FILENAME;
const IMAGES_DIRNAME = ARCHIVE_IMAGES_DIRNAME;
