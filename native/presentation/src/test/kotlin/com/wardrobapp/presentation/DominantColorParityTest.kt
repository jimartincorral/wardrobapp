 ...
 */
class DominantColorParityTest {

    @Test
    fun `every image averages to the same palette colour`() {
        val failures = mutableListOf<String>()

        for (case in Parity.load("dominant-color.jsonl")) {
            val name = case.string("name")
            val pixels = Base64.getDecoder().decode(case.string("pixels"))
            val expected = case.string("color")
            val actual = dominantGarmentColor(pixels)
    ...
    @Test
    fun `the corpus reaches more than one answer`() {
        // A port that returned black for everything would pass a check whose
        // corpus was all black images, and "black" is exactly the answer a broken
        // average produces.
        val answers = Parity.load("dominant-color.jsonl").map { it.string("color") }.toSet()

        assertTrue(answers.size >= 8, "only ${answers.size} distinct colours in the corpus")
        assertTrue("#000000" in answers, "nothing in the corpus comes out black")
        assertTrue("#FFFFFF" in answers, "nothing in the corpus comes out white")
    }
