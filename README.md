...
- **On-device type suggestions** — "Suggest Type" reads the photo and fills in the category, and the type where the photo was clear enough about it. ML Kit's general labelling model, fetched by Play Services the first time it is asked for rather than shipped in the APK; nothing about a photo leaves the phone. It is a general model, so it knows "Footwear" more often than "Loafers" — a category is a real answer here, narrowing the chips from seventy types to seven, and a photo it recognises nothing in says so rather than guessing.
...
```bash
./gradlew test                    # 522 tests, no Android SDK, seconds
./gradlew :app:testDebugUnitTest  # 70 more, needs the SDK — no emulator
```

The 522 cover the suggestion engine, duplicate detection, colour comparison, pair learning, URL safety and which addresses will be fetched, reading a product page, row normalization against every list-column shape that exists, the two database schemas in the wild, backup validation and its refusal messages, the form rules, what a photo's labels mean in this app's vocabulary, filtering and ordering, the chart arithmetic, and both languages' string resources against each other.

The 70 in `:app` are Robolectric tests, not instrumented ones — what a screen shows, where a file lands, and what another activity is asked for, which is the part no pure module can answer:
...
- **Type suggestions are one general model's opinion.** The labeller knows a few hundred everyday things, of which perhaps forty are clothing, so it reports a category far more often than a type and it will not tell a blouse from a shirt. Two known gaps in the mapping: swimwear has no entry in this app's vocabulary at all, so a bikini suggests nothing rather than being filed under Underwear; and the bare label "Vest" is ignored, because it means a waistcoat in American English and a tank top in British, which are different categories here. Everything the model says is turned into vocabulary by one table, so a better model is a change to one file.
