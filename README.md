...
./gradlew :app:testDebugUnitTest  # 88 more, needs the SDK — no emulator
```
...
The 88 in `:app` are Robolectric tests, not instrumented ones — what a screen shows, where a file lands, and what another activity is asked for, which is the part no pure module can answer:
